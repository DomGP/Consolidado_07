...
export default new Vuex.Store({
  ...
  getters: {
      countTotalCupos: state=>{
        let totalCupos = 0;
        state.cursos.forEach(curso => {
          totalCupos += curso.cupos;
        });
        return totalCupos;
      },
      countTotalInscritos: state=>{
        let totalInscritos =0;
        state.cursos.forEach(curso=>{
          totalInscritos += curso.inscritos;
        });
        return totalInscritos;
      },
      countTotalCuposDisponibles: state => {
